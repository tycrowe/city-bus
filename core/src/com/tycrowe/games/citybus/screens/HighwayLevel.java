...
public class HighwayLevel implements Screen {
    ...
    public void show() {
        ...
        this.director = new GameDirector(am, gameStage, uiStage, playerCar);
        director.addFirstHelper(-135, 0);
    }
    ...
    public void render(float delta) {
		...
		gameCamera.position.x += (playerCar.getCarCenter().x - gameCamera.position.x) * (playerCar.getCarSpeed() * .25) * CityBusGame.CAMERA_LERP * CityBusGame.GAME_DELTA;
        gameCamera.position.y += (playerCar.getCarCenter().y - gameCamera.position.y) * (playerCar.getCarSpeed()* .25) * CityBusGame.CAMERA_LERP * CityBusGame.GAME_DELTA;
        gameCamera.update();
